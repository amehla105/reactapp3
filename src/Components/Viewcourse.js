...
export default class Viewcourse extends React.Component {
  ...
  getdataHandler() {
    axios.get(`http://localhost:8080/course/findallcourses`).then((res) => {
  ...
  deleteHandler = (courseid) => {
    axios
      .delete(`http://localhost:8080/course/deletebycourseid/${courseid}`)
      .then((res) => {
        this.getdataHandler();
      });
  };
  ...
  editHandler = (courseid) => {
    axios
      .get(`http://localhost:8080/course/findbycourseid/${courseid}`)
      .then((res) => {
        const courses = res.data;
        console.log(courses);
        this.setState({
  ...
  updateHandler = () => {
    ...
    axios
      .put(
        `http://localhost:8080/course/updatecoursebyid/${this.state.id}`,
        updatecourse
      )
      .then((res) => {
        this.getdataHandler();
